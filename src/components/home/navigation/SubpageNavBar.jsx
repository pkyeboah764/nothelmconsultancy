...
export default function SubpageNavBar() {
	...
	return (
		<nav className='w-full absolute z-50 top-0 left-0 flex items-center justify-between py-4 sm:py-6 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-20'>
			<Logo />
			...
			<div className='hidden sm:block'>
				<Link 
					to='/' 
					className='bg-[#202962] hover:bg-[#1a2252] text-white font-semibold py-2 px-6 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg'
				>
					Home
				</Link>
			</div>

			{/* Mobile Navigation Menu - Consistent Design */}
			<div
				className={`sm:hidden fixed inset-0 bg-gradient-to-br from-[#202962] via-[#1a2252] to-[#151e47] w-full h-screen transition-all duration-500 ease-in-out z-40 ${
			...
			>
				...
				<div className='h-full overflow-y-auto flex flex-col justify-start pt-20 pb-8'>
					...
					{/* Home Button */}
					<div className={`flex justify-center mb-12 px-8 transition-all duration-700 delay-300 ${open ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
						<Link 
							to='/' 
							onClick={() => setOpen(false)}
							className='w-full max-w-sm group relative overflow-hidden bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 hover:border-white/40 rounded-2xl px-8 py-4 transition-all duration-300 transform hover:scale-105 hover:shadow-2xl text-center'
						>
							<div className='absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700'></div>
							<span className='relative text-2xl font-semibold text-white'>Home</span>
						</Link>
					</div>
			...
			<div className={`block sm:hidden relative z-50 transition-opacity duration-300 ${open ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
